...
export const AGENT_TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'norms_defer',
      description: 'Declare that fulfilling the current request would violate one of the session\'s binding norms. Call this BEFORE the risky action, never in place of a normal approval. The user then decides deliberately: if they override, the deferral is recorded; if they honor it, you must not perform the action.',
      parameters: {
        type: 'object',
        properties: {
          rule: { type: 'string', description: 'The norm rule or section number that would be violated, e.g. "Section 5 — Outward fairness".' },
          request: { type: 'string', description: 'What was actually being requested.' },
        },
        required: ['rule'],
      },
    },
  },
  {
    ...
    function: {
      name: 'recall',
      description: 'Search the private, local memory index of past chats, research reports, and saved procedures. Use it BEFORE redoing discovery work — ideal for "what did we decide", "how did we do this last time", user preferences, and project conventions. Returns cited snippets.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Natural-language question or phrase to recall' },
        },
        required: ['query'],
      },
    },
...
// Tool schema to offer the model for a given mode. In plan mode the
// state-changing tools are left out entirely (enforcement still blocks them).
export function toolsForMode(readonly) {
  if (!readonly) return AGENT_TOOLS;
  return AGENT_TOOLS.filter((t) => !RISKY_TOOLS.includes(t.function.name));
}

// ── System prompt templates ──────────────────────────────────────────────────

const CRITIC_SYSTEM = `You are a rigorous second-opinion reviewer for an AI agent's work. You are not continuing the work — you are judging a draft.

Read the original task and the draft answer (or plan) with fresh eyes. Check, in order of importance:
1. Does the draft actually complete the task, end to end?
2. Factual, logical, or code errors.
3. Skipped steps, unhandled risks, or missing edge cases that genuinely matter.
4. Anything confusing or plainly wrong for the reader.

Do not nitpick style. Do not invent issues to seem thorough — a good draft should pass.

Reply in exactly this shape:
VERDICT: PASS
ISSUES:

or, when the draft truly needs work:
VERDICT: REVISE
ISSUES:
1. (concrete, actionable)
2. ...`;

const REVISION_SYSTEM = `You are the author of a draft answer (or plan). A trusted reviewer has annotated it. Rewrite the draft completely: keep what works, apply the legitimately useful notes, ignore any that are wrong, and output ONLY the improved final version — no preamble, no meta commentary, no mention of the review.`;
...
// ── Change ledger — every reversible Agent change, undoable by entry ───────
// Each entry keeps the snapshot needed to reverse the action exactly. Write
// snapshots of already-large files are skipped (they would balloon memory),
// so not every change is recorded — only the ones we can put back. Keep in
// memory for the server run, capped at the newest entries.
const CHANGE_LEDGER = [];
export const CHANGE_LEDGER_MAX = 200;
const LEDGER_MAX_SNAPSHOT = 400_000;
let CHANGE_SEQ = 0;

export function recordChange(kind, snapshot, label) {
  const id = 'g' + (++CHANGE_SEQ);
  CHANGE_LEDGER.push({ id, kind, label: String(label || kind), when: Date.now(), undone: false, snapshot });
  if (CHANGE_LEDGER.length > CHANGE_LEDGER_MAX) CHANGE_LEDGER.shift();
  return id;
}

export function listLedger() {
  return CHANGE_LEDGER.map(({ id, kind, label, when, undone }) => ({ id, kind, label, when, undone }));
}

export function undoChange(workspaceRoot, id) {
  let entry;
  if (typeof id === 'string' && id) {
    entry = CHANGE_LEDGER.find((e) => e.id === id);
    if (!entry) return { ok: false, error: 'That change was not found' };
    if (entry.undone) return { ok: false, error: 'That change is already undone' };
  } else {
    for (let i = CHANGE_LEDGER.length - 1; i >= 0; i--) {
      if (!CHANGE_LEDGER[i].undone) { entry = CHANGE_LEDGER[i]; break; }
    }
    if (!entry) return { ok: false, error: 'Nothing to undo' };
  }
  const outcome = undoSnapshot(workspaceRoot, entry);
  if (!outcome) return { ok: false, error: 'That change could not be undone', id: entry.id };
  entry.undone = true;
  return { ok: true, id: entry.id, kind: entry.kind, ...outcome, message: 'Undid: ' + entry.label };
}

function undoSnapshot(workspaceRoot, entry) {
  const { kind, snapshot: s } = entry;
  const target = (p) => resolve(workspaceRoot, p);
  try {
    switch (kind) {
      case 'write': {
        const full = target(s.path);
        if (s.existed) {
          mkdirSync(dirname(full), { recursive: true });
          writeFileSync(full, s.content ?? '', 'utf8');
          return { action: 'restored', path: s.path, count: 1 };
        }
        rmSync(full, { force: true });
        return { action: 'removed', path: s.path, count: 1 };
      }
      case 'move': {
        let count = 0;
        for (const move of [...(s.plan || [])].reverse()) {
          try { renameSync(target(move.to), target(move.from)); count += 1; } catch {}
        }
        if (!count) return null;
        return { action: 'moved back', path: (s.plan[0] && s.plan[0].from) || '', count };
      }
      case 'image': {
        let count = 0;
        for (const file of s.files || []) {
          try { rmSync(target(file), { force: true }); count += 1; } catch {}
        }
        if (count) try { rmSync(target(s.dir), { recursive: true, force: true }); } catch {}
        if (!count) return null;
        return { action: 'removed', path: s.dir || '', count };
      }
      case 'research': {
        rmSync(target(s.file), { force: true });
        return { action: 'removed', path: s.file, count: 1 };
      }
    }
  } catch {}
  return null;
...
export function agentWriteFile(workspaceRoot, path, content) {
  ...
  try {
    ...
    if (st.isFile()) {
      ...
      if (st.size <= LEDGER_MAX_SNAPSHOT) previous.content = readFileSync(target, 'utf8');
  ...
  try {
    ...
    if (snapshotable) recordChange('write', previous, (previous.existed ? 'updated file ' : 'created file ') + rel);
...
export function applyOrganizePlan(workspaceRoot, plan) {
  ...
  if (applied.length) recordChange('move', { plan: applied }, `moved ${applied.length} file(s) into folders`);
...
export async function executeTool(name, args, workspaceRoot, { autonomy, signal, readonly, hooks = {} } = {}) {
  ...
  switch (name) {
    case 'recall': {
      if (!hooks.memorySearch) return { error: 'memory is not enabled on this machine' };
      const results = await hooks.memorySearch(String(args.query || ''), { k: 5 });
      if (!results.length) return { content: 'No matching memories.', results: [] };
      return {
        results: results.map((r, i) => ({ n: i + 1, source: r.chunk.title, type: r.chunk.type, snippet: r.chunk.text.slice(0, 500), score: Number(r.score.toFixed(3)) })),
        note: 'Cite sources as [1] [2]… matching the results above.',
      };
    }
    ...
    case 'web_search': {
      ...
      if (!query) return { error: 'No query provided. Pass a query string.' };
      const num = Math.min(Math.max(1, Number(args.num_results) || 5), 10);
      return await webSearch(query, num);
    }

    case 'web_fetch': {
      const url = String(args.url || '').trim();
      if (!url) return { error: 'No url provided. Pass a url to fetch.' };
      try { new URL(url); } catch { return { error: 'Invalid url: ' + url }; }
      return await webFetchPage(url);
    }
...
// Critic Mode: after the agent settles on a draft, a fresh-context reviewer
// pass checks it, and — only when the reviewer finds real issues — a revision
// pass rewrites it. Fail-open by design: a broken critic never blocks a good
// draft from shipping.
async function runCriticPasses(draft, { task, readonly, llmCall, onEvent, iteration, memoryNote = '' }) {
  const kind = readonly ? 'plan' : 'answer';
  const precedent = memoryNote ? `\n\nRelevant precedent from the user's own private memory (treat as binding context when it applies):\n${memoryNote}\n` : '';
  const criticMessages = [
    { role: 'system', content: CRITIC_SYSTEM },
    { role: 'user', content: `Task the agent was given:\n${task}${precedent}\n\nThe agent's draft ${kind}:\n\n${draft}` },
  ];
  onEvent({ type: 'critic_started', iteration });
  let critique = '';
  try {
    const res = await llmCall(criticMessages, [], (delta) => { critique += String(delta || ''); onEvent({ type: 'critic_token', delta }); });
    if (!critique.trim() && res && String(res.content || '').trim()) critique = String(res.content);
  } catch (e) {
    if (e?.name === 'AbortError' || e?.code === 'ABORT_ERR') throw e;
    onEvent({ type: 'critic_complete', verdict: 'skipped', content: '', note: String(e && e.message || e) });
    return { content: draft, verdict: 'skipped' };
  }
  critique = critique.trim();
  const verdictMatch = critique.match(/^\s*VERDICT\s*:?\s*(PASS|REVISE)\b/im);
  const verdict = verdictMatch ? verdictMatch[1].toUpperCase() : (critique.length > 280 ? 'REVISE' : 'PASS');
  if (verdict === 'PASS') {
    onEvent({ type: 'critic_complete', verdict: 'pass', content: critique, iteration });
    return { content: draft, verdict: 'pass', critique };
  }
  onEvent({ type: 'critic_complete', verdict: 'revise', content: critique, iteration });
  onEvent({ type: 'revision_started', iteration });
  let revised = '';
  const revisionMessages = [
    { role: 'system', content: REVISION_SYSTEM },
    { role: 'user', content: `Task:\n${task}\n\nDraft ${kind}:\n\n${draft}\n\nReviewer notes to apply:\n\n${critique}` },
  ];
  try {
    const res = await llmCall(revisionMessages, [], (delta) => { revised += String(delta || ''); onEvent({ type: 'revision_token', delta }); });
    if (!revised.trim() && res && String(res.content || '').trim()) revised = String(res.content);
  } catch (e) {
    if (e?.name === 'AbortError' || e?.code === 'ABORT_ERR') throw e;
  }
  revised = revised.trim();
  if (revised.length >= 40) {
    onEvent({ type: 'revision_complete', content: revised, iteration });
    return { content: revised, verdict: 'revised', critique, draft };
  }
  onEvent({ type: 'revision_complete', skipped: true, iteration });
  return { content: draft, verdict: 'revise_failed', critique };
}
...
export async function runAgentLoop({
  ...
  if (memoryNote) systemPrompt += '\n\n' + memoryNote;
  if (readonly) systemPrompt += '\n\n' + PLAN_SYSTEM;
  if (norms) systemPrompt += `\n\n## Binding Norms
The human and you agreed to the following norms. They are binding for this session — not suggestions.

${norms}

## Deferral
If fulfilling the current user request would violate a norm above, do NOT perform the risky action. Instead call the tool \`norms_defer\` with { "rule": "the exact norm section that would be violated", "request": "what was actually being requested" } and stop. The user then decides deliberately: an override is recorded in norms.log, or the deferral is honored and you must not perform the action. Do not call \`norms_defer\` for mere uncertainty, inconvenience, or normal approval flow — only for a genuine collision with a listed norm.`;
  systemPrompt += `\n\n## Workspace
...
When you have completed the task, provide a final summary without calling any tools.`;
  ...
  onEvent({ type: 'started', task, model, autonomy, mode: readonly ? 'plan' : 'build' });
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    let llmResponse;
    try {
      llmResponse = await llmCall(messages, toolsForMode(readonly), onToken);
    } catch (e) {
      if (e?.name === 'AbortError' || e?.code === 'ABORT_ERR') {
        onEvent({ type: 'cancelled', message: 'Agent cancelled' });
        return { status: 'cancelled', iterations: iteration, thread: threadSnapshot() };
      }
    ...
    if (toolCalls.length === 0) {
      ...
      let final = { content: cleanContent };
      if (critic && cleanContent) {
        if (signal?.aborted) { onEvent({ type: 'cancelled', message: 'Agent cancelled' }); return { status: 'cancelled', iterations: iteration, thread: threadSnapshot() }; }
        try { final = await runCriticPasses(cleanContent, { task, readonly, llmCall, onEvent, iteration, memoryNote }); }
        catch (e) {
          if (e?.name === 'AbortError' || e?.code === 'ABORT_ERR') { onEvent({ type: 'cancelled', message: 'Agent cancelled' }); return { status: 'cancelled', iterations: iteration, thread: threadSnapshot() }; }
          final = { content: cleanContent, verdict: 'skipped' };
        }
      }
      finalMessages.push({ role: 'assistant', content: final.content });
      onEvent({
        type: 'completed',
        message: final.verdict === 'revised' ? 'Task completed (reviewed)' : 'Task completed',
        content: final.content,
        iterations: iteration,
        ...(final.critique ? { critique: final.critique } : {}),
        ...(final.draft ? { draft: final.draft } : {}),
        ...(final.verdict ? { verdict: final.verdict } : {}),
      });
      return {
        status: 'complete', content: final.content, iterations: iteration, thread: threadSnapshot(),
        ...(final.critique ? { critique: final.critique } : {}),
        ...(final.draft ? { draft: final.draft } : {}),
      };
    }
    ...
    for (const tc of toolCalls) {
      ...
      // Anti-loop guard: block an exact repeat of the previous tool call.
      const callKey = JSON.stringify([toolName, Object.keys(toolArgs).sort().map((k) => [k, toolArgs[k]])]);
      if (callKey && callKey === lastToolCall) {
        onEvent({ type: 'tool_call', name: toolName, arguments: toolArgs, iteration, needs_approval: false, repeated: true });
        const hint = { blocked: true, repeated: true, error: 'This is an exact repeat of the previous tool call.' };
        onEvent({ type: 'tool_result', name: toolName, result: hint });
        messages.push({
          role: 'user',
          content: `You just ran ${toolName}(${JSON.stringify(toolArgs)}) and received its result. That call was blocked because it repeats the previous call exactly. Do NOT run it again. Either call a DIFFERENT tool with new arguments, or write your final answer now without calling any tools.`,
        });
        continue;
      }
      ...
      const normsDefer = toolName === 'norms_defer';
      const blockedByReadonly = readonly && RISKY_TOOLS.includes(toolName);
      const needsApproval = normsDefer || (!blockedByReadonly && requiresApproval(autonomy, toolName));
      ...
      if (needsApproval) {
        ...
        const approved = await new Promise((resolve) => {
          onEvent({
            ...
            arguments: toolArgs,
            resolve,
            kind: normsDefer ? 'norms' : 'tool',
            rule: typeof toolArgs.rule === 'string' ? toolArgs.rule : '',
          });
        });
        ...
        if (!approved) {
          if (normsDefer) {
            const toolResult = {
              ...
              deferred: true,
              rule: toolArgs.rule || '',
              error: 'The user honored this deferral. Do not perform the original request.',
            };
            onEvent({ type: 'tool_result', name: toolName, result: toolResult });
            messages.push({
              role: 'user',
              content: `Decision for ${toolName} (rule "${toolArgs.rule || ''}"): HONORED by the user. Do NOT perform the original request — explain the deferral to the user instead.`,
            });
            continue;
          }
        ...
        if (normsDefer) {
          const toolResult = { overridden: true, rule: toolArgs.rule || '', request: toolArgs.request || '' };
          onEvent({ type: 'tool_result', name: toolName, result: toolResult });
          messages.push({
            role: 'user',
            content: `Decision for ${toolName} (rule "${toolArgs.rule || ''}"): OVERRIDDEN deliberately by the user. The override was recorded in norms.log. Proceed exactly as originally requested.`,
          });
          continue;
        }
      ...
      const result = await executeTool(toolName, toolArgs, workspaceRoot, { autonomy, signal, readonly, hooks });
      onEvent({ type: 'tool_result', name: toolName, result });

      // Add tool result to messages
      const rawSummary = typeof result === 'string' ? result : JSON.stringify(result);
      const MAX_TOOL_RESULT = 2000;
      const resultSummary = rawSummary.length > MAX_TOOL_RESULT
        ? rawSummary.slice(0, MAX_TOOL_RESULT) + `\n…[truncated, ${rawSummary.length - MAX_TOOL_RESULT} chars omitted]`
        : rawSummary;
      messages.push({
        role: 'user',
...
export async function webSearch(query, numResults = 5) {
  ...
  let lastError = null;
  for (let attemptIdx = 0; attemptIdx < 2; attemptIdx += 1) {
    try {
  ...
  return [{ error: lastError?.message || 'Search failed' }];
}
...
export async function webFetchPage(url) {
  ...
  return { error: lastError?.message || 'Failed to fetch page', url };
}
