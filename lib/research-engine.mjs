...
constructor({ dataDir, complete, fetchImpl = fetch, resolveHost = (hostname) => lookup(hostname, { all: true }), searxngUrl = '', denyEgress = false }) {
  ...
  start({ query, model, mode = 'quick' }) {
    ...
    const job = { id, query: query.trim(), model, mode, status: 'running', phase: 'planning', message: 'Planning focused searches…', round: 0, rounds: profile.rounds, sources: [], findings: [], queries: [], roundFindings: {}, report: '', error: '', startedAt: Date.now(), completedAt: 0, controller: new AbortController() };
  ...
  resume(id) {
    ...
    const job = {
      ...
      report: '', error: '', startedAt: saved.started_at || Date.now(), completedAt: 0,
